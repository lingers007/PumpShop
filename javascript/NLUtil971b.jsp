...
function old_validate_field(field, type, doalert, autoplace, minval, maxval, mandatory, separator)
{
	...
	{
        validflag = /^([a-zA-Z0-9_]+)(\.([a-zA-Z0-9_]+))*$/.test(field.value);
        if (!validflag && doalert)
            alert("App ID can contain only digits, alphabetic characters, underscore \"_\" or dot \".\" with no spaces. It should not start or end with a dot.");
        else
    ...
    if (time != null && new String(time).length != 0 && new String(time).search(/\S/) >= 0)
    {
		...
		if (delimitors != null)
        {
            switch (delimitors.rcase)
            {
                ...
                case 1:
                {
                    m = TIME_FORMAT_WITH_POSSIBLE_AMPM_PREFIX.exec(time);
                    if (m !==  null)
                    {
